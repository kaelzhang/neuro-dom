...
// from jQuery
var REGEX_MATCH_SIMPLE_SELECTOR = /^(\w*)(?:#([\w\-]+))?(?:\.([\w\-]+))?$/;
...
var event_storage = (storage.events = {});
// cleaner
...
var storage = event_storage[id];
    ...
    if(storage){
        lang.each(storage, function(v, key){
...
// from jQuery
function checkRelatedTarget(el, event){
...
function dispatch(el, event, type){
    ...
    for(i = live, len = handlers.length; i < len; i ++){
        ...
        if(!condition || condition(el, event)){
            // return false;
...
function addDOMEvent(el, type, selector, fn){
    ...
    if(!handlers){
        ...
        feature.addEvent(el, handlers.base, handlers.handle, handlers.capture);
    }
...
function removeDOMEvent(el, type, selector, fn){
    ...
    while(len --){
        ...
        for(; i < handlers.length; i ++){
            ...
            if(
                ...
                && (
                    ...
                    || !handler.selector && (
                        selector === undefined || selector === false
                    )
                )
            ){
                handlers.splice(i --, 1);
        ...
        if(!handlers.length){
            feature.removeEvent(el, handlers.base, handlers.handle, handlers.capture);
            delete storage[type];
        }
    }
}
...
function DOMEvent(event, win, config){
    ...
    lang.mix(self, event, true, ['shiftKey', 'ctrlKey', 'altKey', 'metaKey']);
    ...
    if (real.indexOf('key') !== -1){
        ...
        doc = feature.getCompactElement(win.document);
        ...
        lang.mix(self, event, true, ['rotation', 'scale', 'targetTouches', 'changedTouches', 'touches']);
...
var is_eventListener_supported = !!window.addEventListener;
...
var Events = {
    ...
    mousewheel: {
        base: ua.mozilla ? 'DOMMouseScroll' : 'mousewheel'
    },
...
extend({
    ...
    on: function (type, selector, fn){
        ...
        if(lang.isString(type)){
            ...
            // .on(type, selector, fn)
            }else if(!lang.isFunction(fn) || !lang.isString(selector)){
                return;
            }
        ...
        // .on(type-map, selector)
            ...
            if(!lang.isString(selector) && arguments.length > 1){
                return;
            }
        
            for(t in type){
                lang.isFunction(type[t]) && addDOMEvent(el, t, selector, type[t]);
            }
        }
    ...
    off: function(type, selector, fn){
        ...
        if(lang.isObject(type)){
            ...
            if(!lang.isString(selector) && len > 1){
                return;
            }
            ...
            // .off(object, string)
            for(t in type){
                lang.isFunction(type[t]) && removeDOMEvent(el, t, selector, type[t]);
            }
            ...
            // ignore:
            // .off(type, selector, x)
            }else if(!lang.isFunction(fn) && len > 2){
                return;
            }
